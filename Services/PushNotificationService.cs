...
namespace Berca_Backend.Services
{
    ...
    public class PushNotificationService : IPushNotificationService
    {
        ...
        public async Task<List<SmartNotificationDto>> GenerateIntelligentNotificationsAsync(int? branchId = null)
        {
            try
            {
                ...
                if (totalValueAtRisk > 10000000) // 10M IDR threshold
                {
                    ...
                    {
                        Type = NotificationTypes.HIGH_FINANCIAL_RISK,
                        Priority = Models.NotificationPriority.Critical,
                        Title = "🚨 High Financial Risk Alert",
                        Message = $"Total inventory value at risk: Rp {totalValueAtRisk:N0} from expiring products",
                        PotentialLoss = totalValueAtRisk,
                        ActionItems = new List<string>
                        {
                            "Review all expiring products immediately",
                            "Implement emergency discount strategy",
                            "Consider bulk transfers to other branches"
                        }
                    });
                }

                return notifications.OrderByDescending(n => n.Priority).ThenBy(n => n.ActionDeadline).ToList();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error generating intelligent notifications");
                return new List<SmartNotificationDto>();
            }
        }
        ...
        public async Task<bool> ProcessNotificationRulesAsync()
        {
            try
            {
                var intelligentNotifications = await GenerateIntelligentNotificationsAsync();
                
                foreach (var smartNotification in intelligentNotifications)
                {
                    // Convert smart notification to standard notification payload
                    var payload = new NotificationPayload
                    {
                        Title = smartNotification.Title,
                        Body = smartNotification.Message,
                        Icon = "/assets/icons/alert.png",
                        Badge = "/assets/icons/badge.png",
                        Url = smartNotification.ActionUrl,
                        Data = new Dictionary<string, object>
                        {
                            ["type"] = smartNotification.Type,
                            ["priority"] = smartNotification.Priority.ToString(),
                            ["actionItems"] = smartNotification.ActionItems ?? new List<string>()
                        }
                    };

                    // Send based on escalation rules
                    if (smartNotification.EscalationRule?.EscalateToRoles != null)
                    {
                        await SendToRolesAsync(smartNotification.EscalationRule.EscalateToRoles, payload);
                    }
                    else
                    {
                        ...
                        await SendBulkNotificationAsync(allUsers, payload);
                    }
                }
        ...
        public async Task<List<EscalationAlert>> ProcessEscalationAlertsAsync()
        {
            try
            {
                ...
                foreach (var notification in overdueNotifications)
                {
                    ...
                    var escalationPayload = new NotificationPayload
                    {
                        Title = $"🚨 ESCALATED: {notification.Title}",
                        Body = $"Critical alert requires immediate attention. Original alert created {(DateTime.UtcNow - notification.CreatedAt).Hours} hours ago.",
                        Icon = "/assets/icons/escalation.png",
                        Badge = "/assets/icons/critical-badge.png",
                    ...
                    var managerIds = managersAndAdmins.Select(u => u.Id).ToList();
                    if (managerIds.Any())
                    {
                        await SendBulkNotificationAsync(managerIds, escalationPayload);
                    }
        ...
        public async Task<NotificationPreferencesDto> GetUserNotificationPreferencesAsync(int userId)
        {
            try
            {
                // Try to read preferences; if the table is missing, return defaults
                ...
                {
                    _logger.LogWarning(ex, "Error accessing UserNotificationPreferences table. Using defaults.");
                }
        ...
        public async Task<bool> SendCriticalExpiryAlertsAsync()
        {
            ...
            {
                var intelligentNotifications = await GenerateIntelligentNotificationsAsync();
                var criticalExpiryAlerts = intelligentNotifications
                    .Where(n => n.Type == NotificationTypes.CRITICAL_EXPIRY)
                    .ToList();

                if (!criticalExpiryAlerts.Any())
                {
                    return true; // No critical alerts to send
                }

                foreach (var alert in criticalExpiryAlerts)
                {
                    var payload = new NotificationPayload
                    {
                        Title = alert.Title,
                        Body = alert.Message,
                        Icon = "/assets/icons/expiry-alert.png",
                        Badge = "/assets/icons/critical-badge.png",
                        Url = alert.ActionUrl,
                        Data = new Dictionary<string, object>
                        {
                            ...
                            ["actionItems"] = alert.ActionItems ?? new List<string>()
                        }
                    };

                    // Send to managers and admins for critical alerts
                    await SendToRolesAsync(new List<string> { "Manager", "Admin" }, payload);
                }

                _logger.LogInformation("Sent {Count} critical expiry alerts", criticalExpiryAlerts.Count);
                return true;
            }
            catch (Exception ex)
            {
