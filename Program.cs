...
using Berca_Backend.Infrastructure;
...
{
    options.WorkerCount = 1; // Number of concurrent background jobs
    options.SchedulePollingInterval = TimeSpan.FromMinutes(1); // How often to check for scheduled jobs
});
...
app.UseHangfireDashboard("/hangfire", new DashboardOptions
{
    ...
    DashboardTitle = "Toko Eniwan - Background Jobs"
});
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        logger.LogInformation("🔄 Setting up Hangfire recurring jobs...");

        // 1. Daily Calendar Auto-Generation Job
        // Runs every day at 00:00 (midnight) - Generates events for next 30 days
        RecurringJob.AddOrUpdate<IBackgroundJobService>(
            "calendar-auto-generate-daily",
            service => service.AutoGenerateCalendarEventsDaily(),
            Cron.Daily(0, 0),
        ...
        logger.LogInformation("✅ Scheduled: Daily calendar auto-generation at 00:00");

        // 2. Weekly Cleanup Job
        // Runs every Sunday at 02:00 - Cleans up old completed events
        RecurringJob.AddOrUpdate<IBackgroundJobService>(
            "calendar-cleanup-weekly",
            service => service.CleanupOldAutoGeneratedEvents(),
        ...
        logger.LogInformation("✅ Scheduled: Weekly cleanup every Sunday at 02:00");

        // 3. Hourly Reminder Job
        // Runs every hour - Sends pending event reminders
        RecurringJob.AddOrUpdate<IBackgroundJobService>(
            "calendar-reminders-hourly",
            service => service.SendPendingEventReminders(),
            Cron.Hourly(),
            new RecurringJobOptions
            {
                TimeZone = TimeZoneInfo.Local
        ...
        logger.LogInformation("✅ Scheduled: Hourly reminders check");

        // Enqueue a test job to verify Hangfire is working
        BackgroundJob.Enqueue<IBackgroundJobService>(service => service.TestBackgroundJob());
        logger.LogInformation("✅ Queued: Test job to verify Hangfire is working");

        logger.LogInformation("🎯 All background jobs scheduled successfully");
        logger.LogInformation("📊 Access Hangfire Dashboard at: http://localhost:5171/hangfire");
    }
    catch (Exception ex)
    {
        logger.LogError(ex, "❌ Failed to setup background jobs");
